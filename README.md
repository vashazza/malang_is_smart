...
ESP32-S3 기반 손 재활 디바이스를 위한 웹앱 프로토타입.
**하드웨어가 아직 없어도 가짜(Mock) 기기로 모든 기능 동작 가능.**
펌웨어 완성 시 BLE 어댑터만 교체하면 그대로 연결됨.
...
기본은 **Mock 모드**라서 실제 기기 없이도 모든 화면 동작.
...
│   ├── mockDevice.ts   # 가짜 기기 (50Hz 합성 압력 데이터 생성)
│   ├── webBluetooth.ts # 실제 ESP32와 통신 (펌웨어 완성 후 활성화)
│   └── index.ts        # USE_MOCK 토글로 Mock/Real 선택
...
├── pages/
│   ├── Home.tsx        # 기기 연결 + 운동 모드 선택
│   ├── Session.tsx     # 실시간 파형 + 반복 카운트
│   ├── Result.tsx      # 세션 점수 + 상세
...
`src/ble/index.ts`의 `USE_MOCK` 상수, 또는 환경변수로 제어:

```bash
# .env.local 파일 만들고
VITE_USE_MOCK_BLE=false  # 실제 Web Bluetooth 사용
```
...
- [x] 5개 화면 골격
- [x] DB 스키마
- [ ] **Mock 모드로 전체 흐름 확인** ← `npm run dev` 후 한번 돌려보기
...
### Phase 2 — 펌웨어팀과 합의
- [ ] `docs/ble_protocol.md` 리뷰 후 v1.0 확정
- [ ] UUID, 패킷 포맷, 명령 코드 펌웨어에 동일하게 반영
- [ ] 50Hz가 적당한지 (압력 동적 범위 vs BLE 대역폭)
- [ ] 패킷 손실 정책

### Phase 3 — 실 하드웨어 통합
- [ ] ESP32-S3에 펌웨어 플래시
- [ ] `VITE_USE_MOCK_BLE=false` 설정
- [ ] Chrome으로 `chrome://bluetooth-internals` 에서 디바이스 확인
...
- 데모 데이터 초기화: DevTools > Application > IndexedDB > smart-mallangi 삭제.

### 신호처리 임계치
- `src/signal/repCount.ts`의 `DEFAULT_REP_OPTIONS` 값(enterThreshold=1200 등)은
  Mock 데이터 기준. 실제 Velostat 캘리브레이션 후 재조정 필요.
